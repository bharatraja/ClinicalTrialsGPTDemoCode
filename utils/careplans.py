...
class CarePlan():
    ...
    @staticmethod
    async def getCareplanssForPatient(request:Request):
        
        
        #connect to db
        conn= U.getConnection(request)
        conditions: List[asyncpg.Record]=await conn.fetch('SELECT * FROM careplans WHERE patient=$1', request.match_info['id'])
        a=[]
        for row in conditions:
            a.append('"' + row['description'].strip() + ", " +  row['reasondescription'].strip() +' ( from- ' + str(row['start']) + ", to- " + str(row['stop']) + ')"')
        
        str1= '"Careplan": [' + ', '.join(set(a)) + "]"
        return str1

async def main():
    await CarePlan.getCareplanssForPatient("b9c610cd-28a6-4636-ccb6-c7a0d2a4cb85")
